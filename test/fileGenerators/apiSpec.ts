...
describe('Suite for testing ApiFileOperation class', () => {
  ...
  it('should throw an error when trying to initialize models for unknown database', () => {
    const apiFileOperations: ApiFileOperations = new ApiFileOperations('api.test');
    try {
      apiFileOperations.initializeModels('', schemaMocks.PROCESSED_SCHEMA_ONE_TABLE);
    } catch (error) {
      assert.equal(error.message, 'Dialect not supported');
    }
  });
  ...
  it('should throw an error when trying to initialize repositories for unknown database', () => {
    const apiFileOperations: ApiFileOperations = new ApiFileOperations('api.test');
    try {
